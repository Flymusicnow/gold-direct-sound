...
export default function SupporterBadge({
  ...
  if (variant === "mini") {
    return (
      <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${config.bgColor} ${config.borderColor} border`}>
        <Icon className={`h-3 w-3 ${config.color}`} />
        <span className={`text-xs font-medium ${config.color}`}>{level.toUpperCase()}</span>
      </div>
    );
  }
  ...
  return (
    <div className={`flex items-center gap-3 p-4 rounded-lg border ${config.bgColor} ${config.borderColor}`}>
      ...
      <div className="flex-1">
        ...
        {score !== undefined && (
          <p className="text-sm text-muted-foreground">{score} support points</p>
        )}
      </div>
    </div>
  );
