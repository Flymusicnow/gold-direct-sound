...
export function useContextualReport() {
  ...
  const collectContext = async (userNote?: string): Promise<AIContext> => {
  ...
  const submitReport = async (userNote: string): Promise<boolean> => {
    if (!user) return false;
    
    setIsSubmitting(true);
    
    try {
      const aiContext = await collectContext(userNote);
      ...
      // Create summary from user note or default
      ...
      const { error } = await supabase.rpc('upsert_inbox_message', {
        ...
        _title: `Problem rapporterat från ${location.pathname}`,
        _type: 'contextual_report',
        _summary: summary,
        _priority: priority,
        _payload: JSON.parse(JSON.stringify({ ai_context: aiContext }))
      });
